...
namespace Tatva\BugBounty\Controller\Index;
...
class sendEmail extends \Magento\Framework\App\Action\Action {
    ...
    const XML_PATH_EMAIL_BUGBOUNTY = 'tatva/bugbounty/email';
	const XML_PATH_EMAIL_BUGBOUNTY_CUSTOMER = 'tatva/bugbounty/customeremail';
    ...
    public function execute() {
        ...
        $captcha_formID = 'bugbounty_form_captcha';
        ...
        {    
            ...
            if ((count($hd_params) > 0) && !empty($hd_params['name']) && !empty($hd_params['email']) && !empty($hd_params['comment'])) {
                ...
                $attachment_dir = 'bugbounty_files/';
                ...
                $emailTemplateVariables['emailSubject'] = 'BugBounty Form of ';
                $emailTemplateVariables['tableTitle'] = 'BugBounty Form of ';
                    
                $headers = 'MIME-Version: 1.0' . "\r\n";
                $headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
                
                $to_array = $this->_scopeConfig->getValue('contact/customemail/recipient_email_bugbounty', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                $email_to = explode(",",$to_array);
                $cc_array = $this->_scopeConfig->getValue('contact/customemail/cc_recipient_email_bugbounty', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                $email_cc = explode(",",$cc_array);
                $bcc_array = $this->_scopeConfig->getValue('contact/customemail/bcc_recipient_email_bugbounty', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
                $email_bcc = explode(",",$bcc_array);

                try{
                    $this->_template  = $this->_scopeConfig->getValue(self::XML_PATH_EMAIL_BUGBOUNTY, ScopeInterface::SCOPE_STORE, $store);
                    ...
                    /*Customer Email*/
                    $emailTemplateVariables['customerSubject'] = 'BugBounty Form Request submitted';
                    $emailTemplateVariables['customerTitle'] = 'Thank you for submitting a BugBounty to SlideTeam, Our team will contact you soon. Below is a copy of the request you sent for reference.';
                    $this->_template  = $this->_scopeConfig->getValue(self::XML_PATH_EMAIL_BUGBOUNTY_CUSTOMER, ScopeInterface::SCOPE_STORE, $store);
                    $this->_transportBuilder->setTemplateIdentifier($this->_template)
                            ->setTemplateOptions(array('area' => \Magento\Framework\App\Area::AREA_FRONTEND, 'store' => $store))
                            ->setTemplateVars($emailTemplateVariables)
                            ->setFrom(['name' => 'SlideTeam Support', 'email' => 'support@slideteam.net'])
                    ...
                    $response['url'] = $this->url->getUrl('bugbounty/thankyou');
