 ...
 */
namespace Tatva\KillSession\Controller\Adminhtml\User;
    ...
    public function execute()
    {
        ...
        try {
            if (!($isCurrentUserPasswordValid)) {
                throw new AuthenticationException(__('The password entered for the current user is invalid. Verify the password and try again.'));
            }
            ...
            $userCollection = $this->_objectManager->get(\Magento\Security\Model\ResourceModel\AdminSessionInfo\CollectionFactory::class)->create();
                $userCollection->addFieldToFilter('user_id',$userId);
                
                foreach ($userCollection as $user) {
                    //$sessionId = $user->getSessionId();
                    $user->delete();
                }
                //$adminsession->deleteSessionsOlderThen($date);
                //$this->_auth->logout();
            ...
            $this->messageManager->addError(__('The password entered for the current user is invalid. Verify the password and try again.'));
