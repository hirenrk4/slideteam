...
namespace Tatva\Tag\SearchAdapter\Query\Builder;
...
class Match extends \Magento\Elasticsearch\SearchAdapter\Query\Builder\Match
{
    ...
    protected function buildQueries(array $matches, array $queryValue)
    {
        ...
        foreach ($matches as $match) {
            ...
            $searchString =  implode(' ', array_slice(explode(' ', $value), 0, 2));
            ...
            if ($wordCount < 3) {
               ...
               'body' => [
                ...
                $size = sizeof(explode(' ', $value));
                $randomArray = array_slice(explode(' ', $value), 0, $size);
                $seach_k = array_rand($randomArray);
                $random_val = $randomArray[$seach_k];
                $searchString = $searchString.' '.$random_val;
