 ...
 */
namespace Tatva\DownloadableImport\Model\Import;
...
class Product extends \Magento\CatalogImportExport\Model\Import\Product
{
   ...
   private const HASH_ALGORITHM = 'sha256';
     ...
     * @var string
     */
    protected $multiLineSeparatorForRegexp;
    ...
    protected $defaultType = [
        self::COL_TYPE => "downloadable",
        self::COL_ATTR_SET => 'Migration_Default'
    ];
    ...
    public function __construct(
        ...
        \Tatva\Catalog\Block\Category\View $categoryModel,
    ...
    protected function _importData()
    {
        ...
        $this->_eventManager->dispatch('import_product_log_after_successful_importation', ['importData' => $this]);
        return true;
    }
    ...
    public function getImagesFromRow(array $rowData)
    {
        ...
        $baseImage = [];
        $categoryCollection = [];
        $a4categoryCollection = [];
        $gifFlag = false;
        $a4Flag = false;
        $imageFolderPath = $this->getImportImageFile($rowData['sku']);
        $files = glob($imageFolderPath . "/*.gif");
        //Check if animated categories
        $parent_category_id = $this->scopeConfig->getValue('animation/general/categoryId', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        array_push($categoryCollection,$parent_category_id);

        $categoryObj = $this->categoryRepository->get($parent_category_id);
        $subcategories = $categoryObj->getChildrenCategories();
        foreach($subcategories as $subcategorie) {
           array_push($categoryCollection,$subcategorie->getEntityId());
        }
    
        $productCategories = $this->processRowCategories($rowData);
        foreach ($productCategories as $categorie) {
            if(in_array($categorie,$categoryCollection)){
                $gifFlag = true;
                break;
            }
        }
        ...
        foreach ($files as $value) {
            $gifFile  = substr($value,strpos($value,'pics'));
        }

        //Check if a4 parent categories
        ...
        $a4productCategories = $this->processRowCategories($rowData);

        foreach ($a4productCategories as $a4categorie) {
            $_objectManager = \Magento\Framework\App\ObjectManager::getInstance();
            $catModel = $_objectManager->create('Magento\Catalog\Model\Category')->load($a4categorie);
            $parentIds = $catModel->getParentIds();
            
            if(array_intersect($a4categoryCollection, $parentIds))
            {
                $a4Flag = true;
                break;
            }
        ...
        if (!$this->isSkuExist($rowData[self::COL_SKU])) {
            ...
            if (!$a4Flag) {
                if ($rowData['additional_images']) 
                {
                $rowData['additional_images'] = $rowData['additional_images'] . ',instruction/instruction_1.JPG,instruction/instruction_2.JPG,instruction/instruction_3.JPG,instruction/instruction_4.JPG,instruction/instruction_5.JPG';
                $rowData['_media_image'] = $rowData['additional_images'];
                } 
            }
        }
        ...
        foreach ($this->_imagesArrayKeys as $column) {
            if (!empty($rowData[$column])) {
                if($column == 'image' && !empty($files) && $gifFlag == 'true'){
                    $images[$column] = array_unique(
                        array_map(
                            'trim',
                            explode($this->getMultipleValueSeparator(), $gifFile)
                        )
                    );
                }
                    ...
                    if($column == '_media_image' && !empty($files) && $gifFlag == 'true'){
                        $images[$column] = array_merge($images['image'],$images[$column]);  
                    }
    ...
    protected function _saveProducts()
    {
        ...
        $onepagefind = 0;
        //$onepageSkus = array();
        $this->_coreSession->setOnePageFind(0);
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            $onepageCatId = $this->scopeConfig->getValue("resume/general/onepagecategoryName",\Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $documentreportCatId = $this->scopeConfig->getValue("resume/general/document_report_category",\Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $ukraincrisisCatId = $this->scopeConfig->getValue("resume/general/ukrain_crisis_category",\Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $letterheadCatId = $this->scopeConfig->getValue("resume/general/letterhead_category",\Magento\Store\Model\ScopeInterface::SCOPE_STORE);
            $_objectManager = \Magento\Framework\App\ObjectManager::getInstance();

            foreach ($bunch as $rowNum => $rowData) {
                $sku=$rowData[self::COL_SKU];
                $finalsku = substr($sku, 0, 64);
                $productName=$rowData[self::COL_NAME];
                $store_view_code=$rowData[self::COL_STORE_VIEW_CODE];

                $importStartTime[$sku] = date("Y-m-d H:i:s");
                // reset category processor's failed categories array
                $this->categoryProcessor->clearFailedCategories();

                $categoriesArray[$sku]=$rowData['categories'];
                ...
                // Here as we don't need to add images store wise so we choose to use "DEFAULT_STORE_ID" instead of 'default' or 'business_powerpoint_diagrams'
                ...
                foreach($categoryIds as $categoryid)
                {
                    if(empty($categoryid))
                    {
                        throw new LocalizedException(
                                __(
                                    'Category not exist in system. please create category from backend.'
                                )
                            );    
                    }
                }
                ...
                foreach ($categoryIds as $id) {
                    if($this->categoryModel->getCategoryLevel($id) > 2) {
                        $parentCategoryId = $this->categoryModel->getParentCategoryId($id);
                        if($parentCategoryId)
                        {
                            $this->categoriesCache[$rowSku][$parentCategoryId] = true;
                            if($this->categoryModel->getCategoryLevel($parentCategoryId) >  2) {
                                $mainCategoryId = $this->categoryModel->getParentCategoryId($parentCategoryId);
                                $this->categoriesCache[$rowSku][$mainCategoryId] = true;
                            }    
                        }
                    }
                    $this->categoriesCache[$rowSku][$id] = true;
                    $catModel = $_objectManager->create('Magento\Catalog\Model\Category')->load($id);
                    $parentIds = $catModel->getParentIds();
                    
                    if(in_array($onepageCatId,$parentIds) || $id == $onepageCatId || in_array($documentreportCatId,$parentIds) || $id == $documentreportCatId || in_array($letterheadCatId, $parentIds) || $id == $letterheadCatId || in_array($ukraincrisisCatId,$parentIds) || $id == $ukraincrisisCatId ) 
                    {
                        $onepagefind = 1;
                    }
                }
            ...
            //One page and document Report Category manage            
            if($onepagefind == 1)
            {                
                $this->_coreSession->setOnePageFind(1);

                $customconnection = $this->_customresource->getConnection();
                $sql = "TRUNCATE TABLE tatva_custom_category_manage";
                $customconnection->query($sql);
                ...
                foreach ($customCategories as $categoryid) {
                    $catModel = $_objectManager->create('Magento\Catalog\Model\Category')->load($categoryid);
                    $childs = $this->getAllChildren($catModel,true);
                    $dataCat = array();
                    ...
                    foreach($childs as $childId)
                    {
                        ...
                        //$childCat = $_objectManager->create('Magento\Catalog\Model\Category')->setStoreId(0)->load($childId); 
                        //$childCat->setCustomUseParentSettings(1)->save();

                        //$childCat = $_objectManager->create('Magento\Catalog\Model\Category')->setStoreId(1)->load($childId);
                        //$childCat->setCustomUseParentSettings(1)->save();
                    }

                    $finalStr = implode(',',$dataCat);
                    $sql = "INSERT INTO tatva_custom_category_manage (type,category_list) VALUES ('".$categoryid."','".$finalStr."')";
                    $customconnection->query($sql);
                }
            }
            ...
            $importData['name']=$productName;
            $importData['store_view_code']=$store_view_code;
            $importData['categories']=$categoriesArray;

            if(isset($rowData['product_tags']) && $rowData['product_tags']!="")
            {
                 $importData['product_tags']=$rowData['product_tags'];
            }
            $this->_eventManager->dispatch(
    ...
    public function getAllChildren($category,$asArray = false)
    {
    ...
    public function getImportImageFile($sku){
        $filePath = 'import' . '/' . 'pics' .'/' . 'zipimages' . '/' . $sku;
        /** @var \Magento\Framework\Filesystem\Directory\ReadInterface $read */
        $read = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA);
        return $read->getAbsolutePath() . $filePath;
    ...
    protected function _saveStockItem()
    {
        ...
        while ($bunch = $this->_dataSourceModel->getNextBunch()) {
            ...
            $productidsArray = array_unique($productIdsToReindex);
            
            foreach($productidsArray as $pid)
            {
                $importproductids[] = $pid;
            }
        }
        ...
        $logger->info($importproductids);

        //$productidsArray = array_unique($productIdsToReindex);
        $_objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productAction = $_objectManager->create('Magento\Catalog\Model\Product\Action');
        $productAction->updateAttributes($importproductids,['visibility'=>\Magento\Catalog\Model\Product\Visibility::VISIBILITY_BOTH,'status'=>\Magento\Catalog\Model\Product\Attribute\Source\Status::STATUS_ENABLED],0);
        $this->reindexProducts($importproductids);
        $this->_coreSession->setProductIdsArray($importproductids);
    ...
    {
        if($rowData['downloadable_links'])
        {
            $import = $this->_mediaDirectory->getRelativePath('import');
            $downloadLink = explode("=", $rowData['downloadable_links']);
            $downloadFilePath = $import.'/'.$downloadLink[1];

            $mediaPath = $this->filesystem->getDirectoryWrite(DirectoryList::MEDIA);
            
            if(!$mediaPath->isFile($downloadFilePath) && !($this->isSkuExist($rowData['sku'])))
            {
                $this->addRowError('Imported Resource(zip) does not exist in media storage', $rowNum);
            }
        }

        if($rowData[self::COL_TYPE] == 'downloadable') 
        {
            $rowData[self::COL_TYPE]=$this->defaultType[self::COL_TYPE];
            $rowData[self::COL_ATTR_SET]=$this->defaultType[self::COL_ATTR_SET];
        }
    ...
    private function _parseAdditionalAttributes($rowData)
    {
        ...
        $sentence1 = in_array('sentence1=', explode(',', $rowData['additional_attributes']));
        $sentence2 = in_array('sentence2=', explode(',', $rowData['additional_attributes']));
        if ($sentence1){
        $rowData['additional_attributes'] = str_replace('sentence1=,','', $rowData['additional_attributes']);
        }
        if($sentence2){
        $rowData['additional_attributes'] = str_replace('sentence2=,','', $rowData['additional_attributes']);
        }
        $rowData['additional_attributes']=trim($rowData['additional_attributes'], ",");
