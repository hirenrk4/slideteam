...
require(["jquery"], function ($) {
	! function(e) {
	    //var formjsurl = BASE_URL+"jquery.form.js";
	    //void 0 == e.fn.ajaxForm && e.getScript(formjsurl);
	    var a = {};
	    a.fileapi = void 0 !== e("<input type='file'/>").get(0).files, a.formdata = void 0 !== window.FormData, e.fn.uploadFile = function(t) {
	        ...
	        function l(a, t, r) {
	            ...
	            {
				jQuery("#submit").hide();
	            var origFileName = r[o].name;
	            	var fileNameWithoutExt = origFileName.replace(/\.[^.$]+$/, '');
	            	if(fileNameWithoutExt.length > 40)
	            	{
	            		var ext = origFileName.split('.').pop();
						var truncatedFileName = origFileName.substring(0,30)+"....";
						var finalFileName = truncatedFileName + ext;
					}
					else
					{
						var finalFileName = origFileName;
					}
	                var str = origFileName;

	                if(/^[a-zA-Z0-9-_.( ) ]*$/.test(str) == false)
	                {
	                    a.showError && e("<div class='" + a.errorClass + "'><b>" + finalFileName + "</b> " + a.filenameErrorStr + "</div>").appendTo(t.errorLog);
	                    jQuery("#submit").show();
	                }
	            	if (n(t, a, r[o].name))
	                    if (a.allowDuplicates || !d(t, r[o].name))
	                        ...
	                        {
	                          a.showError && e("<div class='" + a.errorClass + "'><b>" + finalFileName + "</b> " + a.sizeErrorStr + "</div>").appendTo(t.errorLog);
	                          //jQuery("#error").val("Error");
	                        }
	                        else if (-1 != a.maxFileCount && t.selectedFiles >= a.maxFileCount) a.showError && e("<div class='" + a.errorClass + "'><b>" + finalFileName + "</b> " + a.maxFileCountErrorStr + a.maxFileCount + "</div>").appendTo(t.errorLog);
	            else {
	                ...
	                var w = new m(t, a),
	                    g = "";
	                g = a.showFileCounter ? t.fileCounter + a.fileCounterStyle + finalFileName : finalFileName, a.showFileSize && (g += " (" + s(r[o].size) + ")"), w.filename.html(g);
	                
	                w.fileNameToAttach.html(finalFileName);
	                
	                var test = origFileName.split(' ').join('_');
	                var data= jQuery('#filesToAttach').val();
					var str = "";
					str = data + test + ",";
					jQuery('#filesToAttach').val(str);
	            ...
	            {
	            	a.showError && e("<div class='" + a.errorClass + "'><b>" + finalFileName + "</b> " + a.duplicateErrorStr + "</div>").appendTo(t.errorLog);	
	            	jQuery("#submit").show();
	            } 
	            else
	            {
	             a.showError && e("<div class='" + a.errorClass + "'><b>" + finalFileName + "</b> " + a.extErrorStr + a.allowedTypes + "</div>").appendTo(t.errorLog);
	             jQuery("#submit").show();
	            }
	            }
	        }
	        ...
	        function h(t, r, o, s) {
	            ...
	            u.change(function() {
	                ...
	                if (this.files) {
	                    ...
	                    str = this.files[g].name;
	                    if(/^[a-zA-Z0-9-_.( ) ]*$/.test(str) == false)
	                    {


	                        return void(o.showError && e("<div class='" + o.errorClass + "'><b>" + str + "</b> " + o.filenameErrorStr + "</div>").appendTo(t.errorLog));
	                    }
	                    else
	                    {
	        ...
	        function v(t, o, s, l, n, d) {
	            var h = {
	                ...
	                beforeSend: function(e) {
	                    s.progressDiv.show(), s.cancel.hide(), s.done.hide(), o.showAbort && (s.abort.show(),jQuery(s.abort).attr("style",""), s.abort.click(function() {
	                    var x = "Are you sure you want to cancel the upload?";
							jQuery('<div>' + x + '</div>').dialog({
						      resizable: true,
						      height:'auto',
						      width:350,
						      title:"Notice",
	                ...
	                success: function(a, r, i) {
	                    ...
	                    n.responses.push(a), s.progressbar.width("100%"), o.showProgress && (s.progressbar.html("100%"), s.progressbar.css("text-align", "center")), s.abort.hide(), o.onSuccess.call(this, l, a, i, s), o.showStatusAfterSuccess ? (o.showDone ? (s.done.show(),jQuery(s.done).attr("style",""), s.done.click(function(){
	                    	//s.statusbar.hide("slow"), s.statusbar.remove()
	                    	}                   
	                    )) : s.done.hide(), o.showDelete ? (s.del.show(), jQuery(s.del).attr("style",""), s.del.click(function() {

							var x = "Are you sure you want to delete?";
							jQuery('<div>' + x + '</div>').dialog({
						      resizable: true,
						      title:"Notice",
						      modal: true,
	        ...
	        var w = e.extend({
	            ...
	            dragDropContainerClass: "ajax-upload-dragdrop input-text",
	            dragDropHoverClass: "state-hover",
	            errorClass: "ajax-file-upload-error",
	            uploadButtonClass: "ajax-file-upload file_upload custom-upload",
	            ...
	            filenameErrorStr: "Special Characters in file name is not supported",
