...
class CategoryLinkManagement implements \Magento\Catalog\Api\CategoryLinkManagementInterface
{
    ...
    public function assignProductToCategories($productSku, array $categoryIds)
    {
        ...
        $tableName1 = $this->_resourceConnection->getTableName('tatva_product_category_remove');

        $sql = "Select * FROM " . $tableName1." Where product_id = ".$productid;
        ...
        foreach (array_diff($assignedCategories, $categoryIds) as $categoryId) {
            ...
            if(empty($count))
            {
                $sql = "Insert into " . $tableName1." (`product_id`,`status`) VALUES (".$productid.",1)";
                $connection->query($sql);
            }
            else
            {
                $sql = "Update " . $tableName1." SET status=1 where product_id=".$productid;
                $connection->query($sql);
            }
