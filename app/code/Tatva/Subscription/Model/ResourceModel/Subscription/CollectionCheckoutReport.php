...
class CollectionCheckoutReport extends AbstractCollection
{
    /**
    ...
    if(count($ids)>0)
    {
      $this->getSelect()->joinRight(
        ...
        ['firstname','lastname','email','main_table.increment_id','main_table.two_checkout_message_id']
        )->where('main_table.subscription_history_id  IN ( '.$s_id.') AND (two_checkout_message_id IS NOT NULL AND two_checkout_message_id != 0) AND (status_success IN ("paid","Requested Unsubscription")) AND (to_date="'.$currentDate.'")')->order('subscription_history_id DESC');
      return $this;
  }
}
