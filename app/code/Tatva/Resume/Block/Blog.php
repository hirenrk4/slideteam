...
class Blog extends Template
{
    ...
    {
        $blogId = $this->scopeConfig->getValue('resume/general/one_resume_blog_id', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
        if(!empty($blogId))
        {           
            $this->_postCollection = $this->_postfactory->create();
    ...
    public function renderSelectedPost($post)
    {
        $html = $this->_listPost->renderResumePost($post);
        return $html;
    }

    public function renderOnePost($post)
    {
        $html = $this->_listPost->renderBlogOnePost($post,"resume");
        return $html;
    }
