...
class sendEmail extends \Magento\Framework\App\Action\Action
{
	...
	const XML_PATH_EMAIL_CUSTMDESIGNSUBCATEGORIES = 'tatva/customresumeservice/email';
	const XML_PATH_EMAIL_CUSTOMRESUMESERVICES = 'tatva/customresumeservice/customeremail';
	...
	public function execute()
	{
		...
		$captcha_formID = 'resume_service_captcha'.$cds_params['blockId'];
        ...
        if ($master_captcha == $cds_params['captcha'][$captcha_formID]){
            if ($master_enable) {
		...
		{
			if((count($cds_params) > 0) && !empty($cds_params['email']) && !empty($cds_params['name']))
			{
				...
				if(sizeof($result) > 0)
				{
					...
					$allowed_file_type = array("pdf", "doc", "docx", "ods", "txt");
					foreach($oneDimensionalArray as $file)
					{
						if(file_exists($file_dir.$file) && $file) 
						{
				...
				$emailTemplateVariables['emailTitle'] = 'Custom Resume Service Request';
				$emailTemplateVariables['tableTitle'] = 'Custom Resume Service Form of ';
				$emailTemplateVariables['emailSubject'] = 'Custom Resume Service Form of ';
				...
				try
				{
					...
					$emailTemplateVariables['customerSubject'] = 'Resume service request submitted';
					...
					$this->_template  = $this->_scopeConfig->getValue(self::XML_PATH_EMAIL_CUSTOMRESUMESERVICES,ScopeInterface::SCOPE_STORE,$store);
					$this->_transportBuilder->setTemplateIdentifier($this->_template)
		            ->setTemplateOptions(array('area' => \Magento\Framework\App\Area::AREA_FRONTEND, 'store' => $store))
		            ->setTemplateVars($emailTemplateVariables)
