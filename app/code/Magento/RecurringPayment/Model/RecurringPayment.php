...
class RecurringPayment extends \Magento\Framework\Model\AbstractModel
{
     ...
     * @param \Magento\Core\Model\ResourceModel\AbstractResource $resource
    ...
    public function isValid()
    {
        ...
        if (!$this->getPeriodUnit()
            || !in_array($this->getPeriodUnit(), array_keys($this->_periodUnits->toArray()), true)
        ) {
        ...
        if($this->getIsTrialAvailable()){
            if ($this->getTrialPeriodUnit()) {
                if (!in_array($this->getTrialPeriodUnit(), array_keys($this->_periodUnits->toArray()), true)) {
    ...
    public function importBuyRequest(\Magento\Framework\DataObject $buyRequest)
    {
        ...
        if ($startDate) {
            ...
            if (!\Zend_Date::isDate($startDate, $dateFormat, $localeCode)) {
                throw new \Magento\Framework\Exception\LocalizedException(__('The recurring payment start date has invalid format.'));
            }
            ...
            if ($options) {
                ...
                if (is_array($options)) {
                    if (isset($options['start_datetime'])) {
                        $startDatetime = $this->_dateTime->formatDate($options['start_datetime'],true);
                        $this->setNearestStartDatetime($this->_dateTime,$startDatetime);
                    }
                }
            }
     ...
     * @return $this
     */
    protected function setNearestStartDatetime($date = null,$date_string = null)
    {
		...
		$store_timezone = new \DateTimeZone('America/Los_Angeles');			
		$GMT_timezone   = new \DateTimeZone('GMT');
		$dateTime = new \DateTime($date, $store_timezone);
		
        $dateTime->setTimezone($GMT_timezone);
        $finalDate = $dateTime->format('Y-m-d H:i:s');
		
		$dateArr = explode(" ",$finalDate);
		$dateStr = $dateArr[0]."T00:00:00";
		
		$this->setStartDatetime($dateStr);
       	//$this->setStartDatetime("2019-05-27T00:00:00");  //static string
        //$this->setStartDatetime($date);
        
        // $this->setStartDatetime("2018-12-21T00:00:00");  //static string
        return $this;
    }
    ...
    public function exportStartDatetime()
    {
		...
		$dateArr = explode("T",$datetime);	
		$finalDate = $dateArr[0]." ".$dateArr[1];
		$timestamp = strtotime($finalDate);
		return date("l, jS \of F Y",$timestamp);	
        
        // return $date->format(\Magento\Framework\Stdlib\DateTime::DATETIME_PHP_FORMAT);   // Y-m-d H:i:s 
        //return $date->format("l, jS \of F Y"); // Eg.Thursday, 20th of December 2018
    }
    ...
    protected function _filterValues()
    {
        ...
        foreach (array(
            ...
            if ($this->hasData($key)) {
                if (!$this->getData($key) || 0 == $this->getData($key)) {
                	...
                	if($key == "init_amount")
					{
                        $cart = $this->getData('cartobj');
                      
						//$objectManager = \Magento\Framework\App\ObjectManager::getInstance();
						//$cart = $objectManager->get('\Magento\Checkout\Model\Cart'); 

						$initPrice = $cart->getQuote()->getGrandTotal();				   
						$this->setData($key, sprintf('%.4F', $initPrice));	
					}
					else
					{
        ...
        if ($this->getStartDatetime()) {
            $this->setNearestStartDatetime($this->_dateTime,$this->getStartDatetime());
        } else {
    ...
    protected function _renderSchedule($periodKey, $frequencyKey, $cyclesKey)
    {
        ...
        $result[] = __('%1 %2 cycle.', $frequency, $this->_periodUnits->toArray()[$period]);
