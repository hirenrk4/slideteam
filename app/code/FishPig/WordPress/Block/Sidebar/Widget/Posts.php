...
class Posts extends AbstractWidget
{
	...
	public function getExcerpt($path){
		return $this->_post->getExcerpt(
            $path,
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE);
	}
	...
	public function getAllPosts(){
        ...
        $posts->getSelect()->order('total_visits DESC');
        $posts->getSelect()->limit(5);
        return $posts;
	} 
	...
	public function getSidebarThumbImage($featured_img){

		$imageUrl = $featured_img->getThumbnailImage();
		
		$baseUrl=$this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB);
		$baseDir = $this->directoryList->getRoot()."/";
		
		$imageUrl = str_replace($baseUrl,$baseDir,$imageUrl);
		$resized_dir =$this->directoryList->getPath('media').'/wp_post_sidebar_resized/';

		// create folder
		if(!file_exists($resized_dir)){
			$flag_t =  $this->io->mkdir($resized_dir, 0777);
		} 
		 
		// get image name
		$imageName = substr(strrchr($imageUrl,"/"),1);
		$imageResized = $resized_dir.$imageName;
		...
		$imageObj->resize(60, 29);
		$imageObj->save($imageResized);
		endif;

		$imageResized = str_replace($baseDir.'pub/', $baseUrl, $imageResized);
