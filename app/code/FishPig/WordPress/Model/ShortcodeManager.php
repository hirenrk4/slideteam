...
class ShortcodeManager
{
	...
	public function addCaptionToDiv($string)
	{
		return preg_replace(
        	'/\[caption([^\]]+)align="([^"]+)"\s+width="(\d+)"\](\s*(<a[^>]+>)*\<img[^>]+>(<\/a>)*)\s*(.*?)\s*\[\/caption\]/i', 
        	'<div\1style="width: \3px" class="wp-caption \2">\4<p class="caption">\7</p></div>', 
        	$string); 
	}
}
