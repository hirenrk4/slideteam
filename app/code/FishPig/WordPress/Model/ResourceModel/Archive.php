		...
		return $this->getConnection()
			->fetchAll(
				"SELECT COUNT(ID) AS post_count, CONCAT(SUBSTRING(post_date, 1, 4), '/', SUBSTRING(post_date, 6, 2)) as archive_date 
					FROM `" . $this->getMainTable() . "` AS `main_table` WHERE (`main_table`.`post_type`='post') AND (`main_table`.`post_status` ='publish') AND (`main_table`.`post_title` !='') GROUP BY archive_date ORDER BY archive_date DESC"
			);
	}
}
