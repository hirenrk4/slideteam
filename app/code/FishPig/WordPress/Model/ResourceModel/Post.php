...
class Post extends AbstractMeta
{
	...
	protected function _getLoadSelect($field, $value, $object)
	{
		...
		$select->joinLeft(
			array('related_tbl' => 'wp_postmeta'),
			"`related_tbl`.`post_id` = `e`.`Id` AND `related_tbl`.`meta_key` = 'related_posts'",
			array('related' => 'meta_value')
		);
		
		$select->joinLeft(
			array('keywords_tbl' => 'wp_postmeta'),
			"`keywords_tbl`.`post_id` = `e`.`Id` AND `keywords_tbl`.`meta_key` = 'Keywords'",
			array('keywords' => "meta_value")
		);
